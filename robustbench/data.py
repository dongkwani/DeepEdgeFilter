...
def load_imagenetc(
    ...
    data_folder_path = Path(data_dir) / 'imagenet-C' / corruptions[0] / str(severity)
...
def load_imagenet200c(
    ...
    data_folder_path = Path(data_dir) / 'imagenet200-C' / corruptions[0] / str(severity)
    imagenet200 = ImageFolder(root=os.path.join(data_folder_path), transform=prepr)
    test_loader = data.DataLoader(imagenet200,
                                  batch_size=n_examples,
                                  shuffle=shuffle,
                                  num_workers=2)
